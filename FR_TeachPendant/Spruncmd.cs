        ...
        public static string WriteCMd(string ip,string cmd)
        {
            ...
            try
            {
                using (WebClient webClient = new WebClient())
                {
                    ...
                    string startTag = "<body topmargin=\"0\">";
                    string endTag = "</body>";
                    ...
                    {
                        throw new Exception("SMON Server not ready");
                    }
                }
            }
            catch (Exception ex)
            {
                // 在发生异常时，返回异常信息
                throw new Exception("SMON " + ex.Message);
            }
        }
    }
}
